...
_As a side note: When you change waveforms and want to avoid clicking artifacts, make sure you make the switch on zero crossing (phase values 0 and 0.5)._

## Exporting WAV files

Let's face it - playing sounds through `aplay` isn't really that useful... Most of you (if anybody) probably came here for sample files. Luckily, playing stuff with `aplay` is literally one step away from having WAV files.
...
And Viola! [Hear it for yourself!](https://github.com/Jacajack/wave-stuff/blob/master/wav_dump/wav/long_1.wav)
